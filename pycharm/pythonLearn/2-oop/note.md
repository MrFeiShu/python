...
- 私有
    ...
    ```buildoutcfg
        ...
        class Person():
              ...
              __age = 18 # 私有成员
    ``` 
    - Python的私有变量不是真私有，是一种成为name mangling的改名策略，可以使用对象._classname__attrbuttename访问（注意：前面是一个下划线，后面是两个下划线）
- 受保护的封装    protected
    - 受保护的封装是将对象成员进行一定级别的封装，然后，在类中或者子类中都可以进行访问，但是在外面都不可以
    - 封装方法：在成员名称前添加一个下划线即可
- 公开    public
    - 公共的封装实际上成员没有任何操作，任何地方都可以访问
    
# 6.2 继承
- 继承就是一个类可以获得另外一个类中的成员属性和成员方法
- 作用：减少代码，增加代码的复用功能，同时可以设置类与类直接的联系
- 继承与被继承的概念
    - 被继承的类叫父类，也叫基类，也叫超类
    - 用于继承的类，叫子类，也叫派生类
    - 继承与被继承一定存在一个is-a关系
...
- 继承的特征
    ...
    - 子类一旦继承父类，则可以使用父类中除私有成员外的所有成员
    - 子类继承父类后并没有将父类成员完全复制到子类中，而是通过引用关系访问调用
    - 子类中可以定义独有的成员属性和方法、
    - 子类中定义的成员和父类成员如果相同，则优先使用子类成员
    - 子类如果想库充父类的方法，可以在定义新方法的同时访问父类成员来进行代码调用。可以使用 [父类名.父类成员] 的格式来调用父类成员，也可以使用 [super().父类成员] 的格式。
- 继承变量函数的查找顺序问题
    - 优先查找自己的变量
    - 没有则查找父类
...
- 构造函数
    ...
    - 如果子类定义了构造函数，则实例化时自动调用构造函数，不查找父类的构造函数
    - 如果子类没有定义，则自动查找父类构造函数（如果父类的构造函数带参数，则构造对象时的参数应该与父类构造函数的参数匹配）
- super
    - super不是关键字，而是一个类
    - super的作用是获取MRO（MethodResolustionOrder）列表中的第一个类
    - super与父类没有任何实质性关系，但通过super可以调用到父类
    - super使用两个方法，参见构造函数调用父类构造函数
...
- 菱形继承/钻石继承问题
    ...
    - 关于多继承的MRO
        ...
        - python本身采用C3算法用于计算多继承的菱形继承进行的结果
        - MRO列表的计算原则
            - 子类永远在父类前面
            - 如果多个父类，则根据继承语法中括号内的书写顺序排序
            - 如果多个父类继承同一个爷爷类，孙子类中只会选取继承语法中括号中的第一个父类的父类，作为爷爷类 
## 6.3 多态
- 多态就是同一个对象在不同情况下有不同的状态出现
- 多态不是语法，是一种设计思想
- 多态性：一种调用方式，不同的执行效果
- 多态：同一事物的多种形态。动物分为人、狗、猪等
- [多态和多态性](https://www.cnblogs.com/luchaungao/p/6739557.html)
...
- 优点
    ...
    - 可以方便的组织和维护不同功能组件的划分
    - 可以根据需要任意调整功能类的组合
    - 可以避免创建很多新的类而导致类的继承混乱
# 7 类相关函数
- issubclass:检测是否为类的子类
- isinstance:检测一个对象是否是一个类的实例
- hasattr:检测一个对象是否有指定成员
- getattr:get attr
- setattr:set attr
- delattr:delete attr
- dir:获取对象的成员列表
...
# 8 类成员描述符（属性）
- 类的成员描述符是为了在类中的成员对类的成员属性进行相关操作而创建的一种方式
    - get:获取属性
    - set：设置属性
    - delete：删除属性
...
- 操作类
    ...
    - `__call__`:当对象函数使用的时候触发
    - `__str__`:当对象被用作字符串使用的时候触发
    - `__repr__`:返回字符串
    - `__str__`跟`__repr__`的区别？？？
- 描述符相关
    - `__set__`
    - `__get__`
    - `__delete__`
